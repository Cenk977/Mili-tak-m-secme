...
**Interfaces:**
- Consumes:
  - GET / from panel/serve.py (Task 5)
  - POST /upload to panel/serve.py (Task 5)
  - GET /api/ranking from panel/serve.py (Task 5)
  - POST /clear to panel/serve.py (Task 5)
- Produces:
  - HTML5 dashboard served at http://localhost:8765/

## Step 1: Write `panel/index.html` — Complete Implementation

Create `panel/index.html` with this exact code:
...
Once Task 5 (HTTP server) is deployed, test that the dashboard loads in a browser:
...
sleep 2
# Open browser to http://localhost:8765 and verify dashboard displays
```

Expected:
- Dashboard loads with header "Milli Takım Seçme 2026"
- Upload area visible with drag-and-drop UI
- Filter controls (birth year, gender) present
- Empty state message displayed initially
- No console errors
...
git commit -m "feat: add dashboard HTML with upload and ranking display"
```
